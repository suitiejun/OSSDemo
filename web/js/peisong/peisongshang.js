Ext.define("js.peisong.peisongshang",{
    ...
    initComponent : function(){
        ...
        Ext.apply(this, {
            ...
            columns : [
                ...
                {text :"配送商编码",dataIndex:'deliveryId',menuDisabled:true,align : "center"},
                {text:'配送商名称',dataIndex:'deliveryName',menuDisabled:true,align : "center"},
            ...
            items:[
                {
                    ...
                    items:[
                        {fieldLabel:'配送商编码',name:'classinfo.deliveryId'},
                        {fieldLabel:'配送商名称',name:'classinfo.deliveryName'},
                                    ...
                                    form.submit({
                                        ...
                                        success:function(form,action){
                                            ...
                                            if(msgcontent.msg){
                                                ...
                                                Ext.getCmp('grid05').store.reload();
                                                return;
                                            }
                                            Ext.Msg.alert('系统提示',msgcontent.message);
                                        },
    ...
    doselect: function () {
        ...
        Ext.getCmp("grid05").store.load({params:{text:Ext.getCmp("text1").getValue()}});

    },
    dodeleteall:function(){
        var record= Ext.getCmp("grid05").getSelectionModel().getSelection();
        if(record.length>0){
            var list='';
            ...
            Ext.Msg.show({
                ...
                fn:function(btn){
                    if(btn=='yes'){
                        Ext.Ajax.request({
                            url: '/dodeleteall05?ids=' + list,
                            success: function (response) {
                                var msg = Ext.JSON.decode(response.responseText);

                                var totalCount = Ext.getCmp('grid05').store.getTotalCount(); // 所有的记录数，不单单是当前页展示的数据
                                var pageSize = Ext.getCmp('grid05').store.pageSize; // 一页上面展示的记录条数
                                var curPage = Ext.getCmp('grid05').store.currentPage; // 当前页码
                                ...
                                {
                                    Ext.getCmp('grid05').store.currentPage-1;
                                    Ext.getCmp('grid05').store.loadPage(Ext.getCmp('grid05').store.currentPage-1);
                                }
                                Ext.MessageBox.show({
                                    title: '成功',
                                    msg: msg.message,
...
Ext.define('Util',{
    statics: {
        doupdate:function (){
            var record=Ext.getCmp('grid05').getSelectionModel().getSelection()[0];
            Ext.create('Ext.window.Window',{
                id:'message',
                title:'信息插入',
                width:300,
                ...
                items:[
                    {
                        ...
                        items:[
                            {fieldLabel:'配送商编码',name:'classinfo.deliveryId',value:record.get("deliveryId")},
                            {fieldLabel:'配送商名称',name:'classinfo.deliveryName',value:record.get("deliveryName")},
                ...
                items:[
                    {
                        ...
                        items:[
                            ...
                            {fieldLabel:'配送商编码',name:'classinfo.deliveryId',value:record.get("deliveryId"), readOnly:true},
                            {fieldLabel:'配送商名称',name:'classinfo.deliveryName',value:record.get("deliveryName"), readOnly:true},
